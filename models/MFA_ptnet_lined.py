...
from lib.pointnet.pointnet import ptnet, ptnet_kd


class MfaPtnetSsnLinedXyz(nn.Module):
    def __init__(self,
                 feature_dim,
                 nspix,
                 n_iter=10,
        ...
        self.mfa = MultiScaleFeatureAggregation([32, 64, 64],
                                                ...
                                                32, self.channel,
                                                [0.2, 0.3, 0.4])
        self.ptnet = ptnet(self.feature_dim,input_channel = self.feature_dim+self.channel)

    def forward(self, x):
        mfa = self.mfa(x)
        mfa_with_xyz = torch.cat((x, mfa), dim=1)
        net = self.ptnet(mfa_with_xyz)
        return soft_slic_pknn(net, net[:, :, :self.nspix]), mfa


class MfaPtnetSsnLined(nn.Module):
    def __init__(self,
                 feature_dim,
                 nspix,
                 n_iter=10,
        ...
        self.mfa = MultiScaleFeatureAggregation([32, 64, 64],
                                                ...
                                                32, self.channel,
                                                [0.2, 0.3, 0.4])
        self.ptnet = ptnet_kd(self.feature_dim, self.feature_dim)
