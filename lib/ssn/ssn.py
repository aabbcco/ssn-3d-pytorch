...
def soft_slic_all(point, seed, n_iter=10, k_faces=8):
    ...
    """
    for _ in range(n_iter):
        dist_matrix = point.new(point.shape[0], seed.shape[-1],
                                point.shape[-1]).zero_()
        for i in range(seed.shape[-1]):
            initials = seed[:, :,
                            i].unsqueeze(-1).repeat(1, 1, point.shape[-1])
            dist_matrix[:, i, :] = -(pairwise_distance(point, initials) *
                                     pairwise_distance(point, initials))
    ...
    for iter in range(n_iter):
        ...
        for batch_idx in range(B):
            for seed_i in range(seed.shape[-1]):
                ...
                seeed = seed[batch_idx, :,
                             seed_i].unsqueeze(0).repeat(pointt.shape[0], 1)
...
def soft_slic_pknn(point, seed, n_iter=10, k_point=256):
    ...
    if point.device == 'cpu':
        ...
        knn = knn_indices_func_gpu
    for _ in range(n_iter):
        dist_matrix = point.new(B, seed.shape[-1], point.shape[-1]).zero_()
