...
__global__ void forward_kernel(
    const scalar_t *__restrict__ pixel_features,                  //deep features
    const scalar_t *__restrict__ spixel_features,                 //superpixel center
    const scalar_t *__restrict__ spixel_indices,                  //spix from previous SSN iter of random distributed
    scalar_t *__restrict__ dist_matrix,                           //memory for output
    int batchsize, int channels, int num_pixels, int num_spixels, //batch size,feature dim,num_pix->num_point,num->spix
    int num_spixels_w, int num_spixels_h)                         //spix width and spix length?
{
    int index = blockIdx.x * blockDim.x + threadIdx.x; //thread distribution
    if (index >= batchsize * num_pixels * 9)           //9 affinity?
        return;                                        //distribute thread to each pixel->spix center

    int cp = channels * num_pixels;  //9 channels for each pixel?
    int cs = channels * num_spixels; //9 channels for each spix. cal the index?

    int b = index % batchsize;                      //index in a batch
    int spixel_offset = (index / batchsize) % 9;    //where the affinity spix in
    int p = (index / (batchsize * 9)) % num_pixels; //pixel index in a batch?
