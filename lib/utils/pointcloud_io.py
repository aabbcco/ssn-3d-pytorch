...
class write:
    ...
    def tobcd(data, pointtype, filename):
        ...
        if pointtype == 'xyz' or pointtype == 'xyzl':
            ...
            data[:, 3] = (data[:, 3].astype('uint32') << 16 | data[:, 4].astype(
                'uint32') << 8 | data[:, 5].astype('uint32'))
            np.savetxt(filename, data[:, :4],
                       fmt=formats, header=header, comments='')
    @staticmethod
    ...
    if asdict:
        ...
        if 'rgb' in header.fields:
            datas = np.concatenate(
                (datas, unpack_rgb(data[:, 3+pointer])), axis=-1)
            pointer += 1
        if 'label' in header.fields:
            datas = np.concatenate(
                (datas, np.expand_dims(data[:, 3+pointer], axis=-1)), axis=-1)
            pointer += 1
    ...
    if(is_new):
        for i,hash_idx in enumerate(hashmap):
            for j,hash_idxx in enumerate(hash_idx):
                if(0!=hash_idxx):  label_pixel[i] += min(pixel_count[j]-hash_idxx,hash_idxx)
